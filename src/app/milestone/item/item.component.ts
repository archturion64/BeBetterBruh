...
@Component({
  ...
  template: `
    <div class="text-sm breadcrumbs m-5">
      <ul>
        <li><a routerLink="/milestones">{{'milestone.listTitle' | translate}}</a></li> 
        <li>{{'milestone.itemTitle' | translate}}</li>
      </ul>
    </div>
    ...
    @if (store.detailsLoading()) {
      ...
      <div class="m-5 flex flex-col">
        ...
        <div class="m-10 grid grid-cols-8 md:grid-cols-2  gap-4">
          <button type="button" class="btn" routerLink="/milestones">{{'milestone.backBtn' | translate}}</button>
